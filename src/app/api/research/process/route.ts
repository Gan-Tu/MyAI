...
import { compileReport, generatePlan, performSearch, summarizeFindings } from '@/lib/research';
...
export async function POST(request: NextRequest) {
  ...
  if (res.rows.length === 0) {
    return NextResponse.json({ message: 'No pending session or already processing' });
  }
  const session = res.rows[0];

  try {
    // Perform async operations outside the transaction
    ...
    if (session.current_step === 0) {
      ...
      queries = [
        { text: 'UPDATE research_sessions SET processing = true WHERE id = $1', values: [session.id] },
        {
          ...
          values: [JSON.stringify(subTopics), JSON.stringify(progress), session.id],
        },
      ];
      ...
      if (index < subTopics.length) {
        ...
        const { summary, needsMoreResearch } = await summarizeFindings(searchResults);
        const summaries = session.summaries || [];
        summaries.push({ subTopic, summary });
        progress.push({ message: `Needs more research on: ${needsMoreResearch}` });
        queries = [
          { text: 'UPDATE research_sessions SET processing = true WHERE id = $1', values: [session.id] },
          {
            text: 'UPDATE research_sessions SET summaries = $1, sub_topic_index = $2, progress = $3, processing = false WHERE id = $4',
            values: [JSON.stringify(summaries), index + 1, JSON.stringify(progress), session.id],
          },
        ];
        shouldTriggerNext = true;
      } else {
        queries = [
          { text: 'UPDATE research_sessions SET processing = true WHERE id = $1', values: [session.id] },
          { text: 'UPDATE research_sessions SET current_step = 2, processing = false WHERE id = $1', values: [session.id] },
        ];
        shouldTriggerNext = true;
      }
      ...
      queries = [
        { text: 'UPDATE research_sessions SET processing = true WHERE id = $1', values: [session.id] },
        {
          ...
          values: [finalReport, 'completed', JSON.stringify(progress), session.id],
        },
      ];
    }

    // Execute the transaction using the array form
    await sql.transaction(
      queries.map((q) => sql(q.text, q.values))
    );

    // Trigger the next step without awaiting
    if (shouldTriggerNext) {
      triggerNextStep(session.id, request.nextUrl.origin, userId);
    }

    return NextResponse.json({ message: `Processed step for session ${session.id}` });
  } catch (error: any) {
    console.error(error);
    await query(
